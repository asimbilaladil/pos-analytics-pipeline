...
SELECT
    ...
    COALESCE(SUM(oi.pure_sales), 0)                             AS revenue,

    SUM(oi.quantity) FILTER (WHERE o.dining_option = 4)         AS qty_drive_through,
    SUM(oi.quantity) FILTER (WHERE o.dining_option IN (0,1))    AS qty_eat_in,
    SUM(oi.quantity) FILTER (WHERE o.dining_option IN (100,101)) AS qty_third_party,
