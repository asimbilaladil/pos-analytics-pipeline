...
def build_modifier_row(mod: dict, item_id: int, order_id: int,
                       est_id: int, item_date: date) -> tuple:
    return (
        mod["id"],
        item_id,
        order_id,
        est_id,
        extract_id(mod.get("modifier")),
        mod.get("name") or mod.get("modifier_name"),
        to_float(mod.get("qty") or mod.get("quantity"), 1.0),
        to_float(mod.get("price")),
...
def run_establishment(context, conn, est_id: int,
    ...
    try:
        ...
        stats = insert_establishment_data(
            conn, est_id, ingestion_date, all_orders, items_by_order
        )
