...
========================
Task 12 orchestrator. Same design as run_task10_remaining.py, retargeted at
backfill_order_history_v2.py. Runs one (establishment, month) chunk per fresh OS
subprocess, strictly sequentially -- this script itself never fetches,
parses, or holds a single OrderHistory row in memory, so it carries none of the
...
Already-successful chunks (checked by backfill_order_history_v2.py itself against
backfill_progress, resource="order_history_v2") are skipped without a real fetch
...
LOG_PATH = f"/root/pos-analytics-pipeline/logs/task12_orchestrator_{RUN_STAMP}.log"
SUMMARY_PATH = "/root/pos-analytics-pipeline/logs/task12_orchestrator_summary.jsonl"
STATUS_PATH = "/root/pos-analytics-pipeline/logs/task12_orchestrator_status.json"
...
def run_chunk(est, start, end):
    cmd = [
        "/usr/bin/time", "-v",
        "python3", "backfill_order_history_v2.py",
...
def main():
    ...
    log(f"=== Task 12 orchestrator start — {len(ESTABLISHMENTS)} establishments x {len(CHUNKS)} chunks = {total_chunks} total ===")
    max_rss_seen = 0
    total_fetched = 0
    n_retries = 0
    n_done = 0
    ...
    log(f"=== Task 12 orchestrator DONE — {n} chunks processed, max_rss_seen={max_rss_seen}KB, total_fetched={total_fetched} ===")
