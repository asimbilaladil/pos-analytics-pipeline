...
def load_modifier_cache(conn) -> dict:
    """Load {modifier_id: name} from the modifiers reference table."""
    with conn.cursor() as cur:
        cur.execute("SELECT id, name FROM modifiers")
        rows = cur.fetchall()
    if not rows:
        log.warning("modifiers table is empty — modifier_name will be NULL. "
                    "Run pipeline.py first to populate it.")
        return {}
    log.info("Loaded %d modifier names from DB", len(rows))
...
def parse_modifier(mod: dict, order_item_id: int, order_id: int,
                   establishment_id: int, item_date: date,
                   modifier_cache: dict) -> dict:
    ...
    return {
        ...
        "establishment_id": establishment_id,
        "modifier_id":      mid,
        "modifier_name":    modifier_cache.get(mid),   # resolved from modifiers reference table
        "qty":              parse_decimal(mod.get("qty") or mod.get("quantity"), 1),
        "modifier_price":   parse_decimal(mod.get("modifier_price")),
...
def process_file(filepath: str, conn, ingestion_date: date,
                 modifier_cache: dict) -> dict:
    """
    ...
    for record in data:
        ...
        for item in items:
            ...
            # Parse modifiers — real Revel field is "modifieritems", not "modifiers"
...
def main():
    ...
    # Load modifier names once — used by all files to populate modifier_name
    ...
    for filepath in files:
        try:
            stats = process_file(filepath, conn, ingestion_date, modifier_cache)
            if stats:
