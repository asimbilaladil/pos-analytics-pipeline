...
========================
Task 11 orchestrator. Same design as run_task10_remaining.py, retargeted at
backfill_payments_v2.py. Runs one (establishment, month) chunk per fresh OS
subprocess, strictly sequentially -- this script itself never fetches,
parses, or holds a single Payment row in memory, so it carries none of the
...
Already-successful chunks (checked by backfill_payments_v2.py itself against
backfill_progress, resource="payments_v2") are skipped without a real fetch
...
LOG_PATH = f"/root/pos-analytics-pipeline/logs/task11_orchestrator_{RUN_STAMP}.log"
SUMMARY_PATH = "/root/pos-analytics-pipeline/logs/task11_orchestrator_summary.jsonl"
STATUS_PATH = "/root/pos-analytics-pipeline/logs/task11_orchestrator_status.json"
...
def run_chunk(est, start, end):
    cmd = [
        "/usr/bin/time", "-v",
        "python3", "backfill_payments_v2.py",
...
def main():
    ...
    log(f"=== Task 11 orchestrator start — {len(ESTABLISHMENTS)} establishments x {len(CHUNKS)} chunks = {total_chunks} total ===")
    max_rss_seen = 0
    total_fetched = 0
    n_retries = 0
    n_done = 0
    ...
    log(f"=== Task 11 orchestrator DONE — {n} chunks processed, max_rss_seen={max_rss_seen}KB, total_fetched={total_fetched} ===")
