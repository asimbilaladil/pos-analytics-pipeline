#!/usr/bin/env python3
"""
run_task11_remaining.py
========================
Task 13 orchestrator. Same design as run_task10_remaining.py, retargeted at
backfill_modifier_items_v2.py. Runs one (establishment, month) chunk per fresh OS
subprocess, strictly sequentially -- this script itself never fetches,
parses, or holds a single ModifierItem row in memory, so it carries none of the
long-lived-process OOM risk Task 09 hit.

Safety thresholds (same as Task 10's orchestrator):
...
Already-successful chunks (checked by backfill_modifier_items_v2.py itself against
backfill_progress, resource="modifier_items_v2") are skipped without a real fetch
-- this script does not duplicate that logic, it just iterates the full
chunk list and lets each subprocess decide. The est=40/2026-03 pilot chunk
is already marked success and will be skipped automatically.
"""
import json
import os
import subprocess
...
LOG_PATH = f"/root/pos-analytics-pipeline/logs/task13_orchestrator_{RUN_STAMP}.log"
SUMMARY_PATH = "/root/pos-analytics-pipeline/logs/task13_orchestrator_summary.jsonl"
STATUS_PATH = "/root/pos-analytics-pipeline/logs/task13_orchestrator_status.json"
...
def run_chunk(est, start, end):
    cmd = [
        "/usr/bin/time", "-v",
        "python3", "backfill_modifier_items_v2.py",
...
def main():
    ...
    log(f"=== Task 13 orchestrator start — {len(ESTABLISHMENTS)} establishments x {len(CHUNKS)} chunks = {total_chunks} total ===")
    max_rss_seen = 0
    total_fetched = 0
    n_retries = 0
    n_done = 0
            ...
            while True:
                ...
                if rss > RSS_LIMIT_KB:
                    ...
                    write_status(state="ABORTED_RSS", chunk=f"est={est} {start}..{end}",
                                 ...
                                 total_fetched=total_fetched, n_retries=n_retries)
                    return 1
                ...
                if result["ok"]:
                    ...
                    write_status(state="RUNNING", chunks_done=n_done, chunks_total=total_chunks,
                                 ...
                                 n_retries=n_retries, last_chunk=f"est={est} {start}..{end}")
                    break
    ...
    log(f"=== Task 13 orchestrator DONE — {n} chunks processed, max_rss_seen={max_rss_seen}KB, total_fetched={total_fetched} ===")
    write_status(state="DONE", chunks_done=n_done, chunks_total=total_chunks,
                 max_rss_seen_kb=max_rss_seen, total_fetched=total_fetched, n_retries=n_retries)
    return 0
