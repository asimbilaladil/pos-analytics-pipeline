...
# Task 16 post-cutover daily sequence. NOT YET ACTIVE — the crontab still
# points at run.sh (this file's pre-existing legacy-writer counterpart).
# Activating cutover means pointing crontab at this file instead (or
# replacing run.sh's content with this one) — nothing here runs until that
# switch happens. run.sh.pre-task16 is the byte-for-byte backup of the
# current run.sh, kept for fast rollback (see Task 16 plan §8).
#
